...
export const createReview = async (req: Request, res: Response): Promise<void> => {
    ...
    } catch (error: unknown) {  // Spécifie que l'erreur est de type 'unknown'
        if (error instanceof Error) {  // Vérifie si l'erreur est une instance d'Error
...
export const updateReview = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const updatedReview = await reviewService.updateReview(reviewId, updatedData);  // Appel au service pour mettre à jour la revue
        if (!updatedReview) {
            res.status(404).json({ message: "Review not found" });
        } else {
